...
ブラウザ上で mp4 を フレーム分割・フレーム合成・モザイク処理を行います。
ローカル環境での起動を想定しており、フレーム分割した jpg ファイルは特定のフォルダに保存されています。

## TL;DR

### bash (Linux / macOS / Git Bash / Termux など)
...
### Windows
1. `windows\install.bat` でインストール
2. `windows\run.bat` で起動
3. `http://localhost:8000/html/` で UI にアクセス
4. **upload** タブ: 動画をアップロード（ファイル選択 または Google Drive URL 貼り付け）
5. **split** タブ: フレーム数を指定して JPG 切り出し
6. **mergeframe** タブ: 複数フレームを平均合成（省略可）
7. **mosaic** タブ: エリア指定してモザイク処理（省略可）
8. **preview** タブ: プレイリスト再生・ZIP ダウンロード
9. `windows\uninstall.bat` でアンインストール
...
- Python 3.10 以上
- bash 実行環境（Linux / macOS / Git Bash / Termux）または Windows CMD
...
### bash (Linux / macOS / Git Bash / Termux) を使用する場合
...
- **venv 確認**: `.venv` が存在しない場合はエラーで終了
- **デフォルト値**: `APP_FILE=server.py`、`PORT=8000`（環境変数で上書き可能）
- **Windows CMD**: `.venv\Scripts\activate.bat` をアクティベートして `python -m fastapi run` を実行
- **bash**: `.venv/bin/activate` または `.venv/Scripts/activate` をアクティベートして `python -m fastapi run` を実行
- 起動後のアクセス先: `http://localhost:8000/html/`
...
- **通常環境**: `.venv` ディレクトリを削除
- **Termux 環境**: `python-numpy` / `python-pillow` / `ffmpeg` / `rust` の個別削除を対話形式で確認後、`.venv` を削除
- **Windows CMD**: `.venv` ディレクトリを削除（Termux 処理なし）
